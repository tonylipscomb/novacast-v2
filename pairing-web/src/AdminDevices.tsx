...
export function AdminDevices({
  ...
  const filtered = useMemo(
    () =>
      devices.filter((device) => {
        const haystack = [
          ...
          providerNameById.get(String(device.managed_provider_id ?? '')),
        ]
          .map(String)
          .join(' ')
          .toLowerCase();
    ...
    [devices, query, status, platform, beta, providerNameById],
  );
  ...
  const exportDevices = () => {
    const safe = filtered.map((device) => ({
      id: device.id,
      public_device_code: device.public_device_code,
      ...
      content_policy: device.content_policy,
      managed_provider_id: device.managed_provider_id,
    }));
    const blob = new Blob(
    ...
    <div className="deviceTableRow">
      <div className="deviceIdentity">
        <span className="deviceAvatar">
          {String(device.platform ?? '').toLowerCase().includes('android') ? '▣' : '◈'}
        </span>
        <div>
      ...
      <div className="deviceActions">
        ...
        <button
          aria-label={`Extend ${name} by 24 hours`}
          title="Extend access +24h"
          onClick={() => onExtend(id, 24)}>
          ↗
        </button>
        <button
