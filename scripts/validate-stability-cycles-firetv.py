"""10-cycle stability validation for Live TV and Movies on Fire TV 10.0.0.179. Validation only."""
import io
import json
import os
import re
...
def prep_environment() -> dict:
    info = {
        'device': DEVICE,
        'metro_project': 'C:\\Users\\tonyl\\Desktop\\novacast-v2',
        'adb_reverse': False,
        'reload': False,
    }
    ...
    if not info['adb_reverse']:
        subprocess.run(['adb', '-s', DEVICE, 'reverse', 'tcp:8081', 'tcp:8081'], check=False)
        reverse_list = subprocess.run(['adb', '-s', DEVICE, 'reverse', '--list'], capture_output=True, text=True).stdout
        info['adb_reverse'] = 'tcp:8081 tcp:8081' in reverse_list
...
def main() -> None:
    ...
    print(f'=== MOVIES {MOVIE_CYCLES} CYCLES ===', flush=True)
    ...
    out_path = f'{OUT}/stability-cycle-results-firetv.json'
