...
export const XTREAM_GUIDE_CHANNEL_PAGE_SIZE = 24;
/** Parallel short-EPG fetches per Guide page. Higher = faster first paint; too high risks provider rate limits. */
const XTREAM_GUIDE_EPG_CONCURRENCY = 6;
export const XTREAM_GUIDE_EPG_LIMIT = 12;
/**
 * Hard ceiling on channels kept in Guide UI memory for a category.
...
export type ProviderLiveCategory = {
  ...
  renderKey: string;
  name: string;
  /** Original provider label before display normalization; used for regional ordering. */
  rawName?: string;
...
export interface ProviderLiveRepository {
  ...
  getTotalChannelCount?(signal?: AbortSignal): Promise<number>;
  /**
   * Fast total that may stop at the Xtream dump ceiling (~10k). Prefer
...
function parseProviderCategoryFields(rawName: unknown, normalizedName: string) {
  const rawLabel = typeof rawName === 'string' && rawName.trim() ? rawName.trim() : normalizedName;
  const fromRaw = parseProviderCategoryLabel(rawLabel);
  if (fromRaw.countryCode || fromRaw.regionMarker) {
    return { ...fromRaw, rawLabel };
  }

  return { ...parseProviderCategoryLabel(normalizedName), rawLabel };
}
...
function dedupeXtreamLiveStreamsByStreamId(streams: XtreamLiveStreamResponse[]) {
  const seenStreamIds = new Set<string>();
  const deduped: XtreamLiveStreamResponse[] = [];

  for (const stream of streams) {
    const streamId = stream.stream_id == null ? '' : String(stream.stream_id).trim();
    if (streamId) {
...
export function createMockProviderRepositories(providerId: string): ProviderRepositories {
  ...
  return {
    ...
    series: {
      async getCategories() {
        return sortProviderCategoriesUsFirst(buildMockSeriesCategories());
      },
...
function partitionXtreamLiveStreamsUsFirst(streams: XtreamLiveStreamResponse[]): XtreamLiveStreamResponse[] {
  if (streams.length <= 1) {
    return streams;
  }
...
export function createXtreamProviderRepositories(client: XtreamClient): ProviderRepositories {
  ...
  let liveStreamCountMayBeTruncated = false;
  ...
  function applyLiveStreamCounts(streams: XtreamLiveStreamResponse[], mayBeTruncated: boolean) {
    ...
    liveStreamCountMayBeTruncated = mayBeTruncated;
    liveCategoryCountsLoaded = true;
  }

  async function ensureLiveCategoryIds(signal?: AbortSignal) {
    if (liveCategoryIds.size) {
      return;
    }
    ...
    categories.forEach((category) => {
      ...
      if (normalized.hasProviderId) {
        liveCategoryIds.add(normalized.id);
      }
    });
  }

  async function recountLiveStreamsByCategory(signal?: AbortSignal) {
    ...
    for (const categoryId of liveCategoryIds) {
      const streams = await client.getLiveStreams(categoryId, signal);
      for (const stream of streams) {
        const streamId = stream.stream_id == null ? '' : String(stream.stream_id).trim();
        if (streamId) {
          if (seenStreamIds.has(streamId)) {
    ...
    cachedLiveStreamCount = seenStreamIds.size;
    liveStreamCountMayBeTruncated = false;
    liveCategoryCountsLoaded = true;
  }

  /**
   * Xtream `get_live_streams` without a category is hard-capped at
   * XTREAM_MAX_ITEMS_PER_RESPONSE (10k). Fast mode uses that dump; accurate mode
   * recounts per category when the dump looks truncated so Account totals can
   * exceed 10k without blocking the rest of catalog sync.
   */
  async function loadLiveStreamIndex(signal?: AbortSignal, options?: { accurate?: boolean }) {
    const wantAccurate = options?.accurate === true;

    if (liveCategoryCountsLoaded && (!wantAccurate || !liveStreamCountMayBeTruncated)) {
      return;
    }
    ...
    if (wantAccurate && dumpLikelyTruncated) {
      await recountLiveStreamsByCategory(signal);
    }
  }
  ...
  const liveRepository: ProviderLiveRepository = {
    ...
    async getApproximateTotalChannelCount(signal) {
    ...
    async getChannel(channelId: string, signal) {
      // Reuse the shared live-stream cache (same as Guide / getChannels) so Favorites
      // does not re-download the full catalog once per channel.
      const streams = await resolveLiveCategoryStreams(undefined, signal);
      const index = streams.findIndex((item) => String(item.stream_id) === channelId);
      if (index < 0) {
        return null;
      }
