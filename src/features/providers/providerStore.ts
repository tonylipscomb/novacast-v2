...
} from '../pairing/pairingTransactionLog.ts';
...
import { markStartupReady } from '../startup/startupReadiness.ts';
  ...
  void bundle.ready
    .then(async () => {
      if (bundle.accountMetadata == null) {
        pairingTransactionSuccess('scheduleProviderAccountValidation', {
      ...
      await runPairingTransactionStep('scheduleProviderAccountValidation.writeState', async () => {
        ...
        if (!current.providers.some((item) => item.id === nextProvider.id)) {
          return;
        }

        await writeState({
          ...current,
          providers: current.providers.map((item) => (item.id === nextProvider.id ? nextProvider : item)),
        });
      }, { providerId: provider.id });
    ...
    .catch((error) => {
      ...
      // Keep the active repository bundle alive after pairing/home navigation.
      // HTTP Xtream providers rely on cleartext traffic; a background metadata fetch
      // must not tear down the bundle the UI is already using.
      setRuntime({
        ...runtime,
...
async function prepareProviderBundle(
  provider: ProviderRecord,
  credentialsOverride?: ProviderCredentialRecord,
  options: { validateAccount?: boolean } = {},
) {
  const validateAccount = options.validateAccount ?? true;
...
/** Legacy hook retained for migration compatibility; pairing credentials never come from public env vars. */
...
export async function connectXtreamProvider(input: {
  ...
  username: string;
  password: string;
}, options: { activate?: boolean; validateAccount?: boolean } = {}) {
  ...
  const normalized = runPairingTransactionStepSync('normalizeXtreamInput', () => normalizeXtreamInput(input), {
    serverId: getProviderServerId(input.baseUrl) ?? 'unknown',
  });
  const previousRuntime = runtime;
  ...
  runtimeLoadPromise = (async () => {
    ...
    try {
      current = await runPairingTransactionStep('connectXtreamProvider.readState', () => readState());
      matching = await runPairingTransactionStep('connectXtreamProvider.findMatchingProvider', () =>
        findMatchingProvider(current.providers, normalized.credentials),
      );
      ...
      await runPairingTransactionStep('setProviderCredentials(staged)', () =>
        setProviderCredentials(stagedCredentialId, normalized.credentials),
      { credentialKey: stagedCredentialId },
      );
      stagedCredentials = true;
      const prepared = await runPairingTransactionStep(
        'prepareProviderBundle',
        () => prepareProviderBundle(draft, normalized.credentials, { validateAccount }),
        { providerId, validateAccount },
      );
      candidateBundle = prepared.bundle;
      ...
      await runPairingTransactionStep('setProviderCredentials(final)', () =>
        setProviderCredentials(providerId, normalized.credentials),
      { credentialKey: providerId },
      );
      credentialsCommitted = true;
      await runPairingTransactionStep('removeProviderCredentials(staged)', () =>
        removeProviderCredentials(stagedCredentialId),
      { credentialKey: stagedCredentialId },
      );
      stagedCredentials = false;

      if (activate) {
        await runPairingTransactionStep('activateRepositoryBundle', async () => {
          activateRepositoryBundle(candidateBundle!);
        }, { providerId });
      } else {
        candidateBundle.invalidate();
      ...
      pairingTransactionFailure('connectXtreamProvider', error, {
        providerId,
        stateCommitted,
        credentialsCommitted,
        credentialCommitAttempted,
        stagedCredentials,
      });
      candidateBundle?.invalidate();
      if (stateCommitted) {
        try {
          await runPairingTransactionStep('connectXtreamProvider.rollback.writeState', () => writeState(current), {
            providerId,
          });
        } catch (rollbackError) {
          pairingTransactionFailure('connectXtreamProvider.rollback.writeState', rollbackError, { providerId });
        }
      }
      if (stagedCredentials) {
        try {
          await runPairingTransactionStep('connectXtreamProvider.rollback.removeProviderCredentials(staged)', () =>
            removeProviderCredentials(stagedCredentialId),
          { credentialKey: stagedCredentialId },
          );
        } catch (rollbackError) {
          pairingTransactionFailure('connectXtreamProvider.rollback.removeProviderCredentials(staged)', rollbackError, {
            credentialKey: stagedCredentialId,
          });
        }
      }
      if (credentialCommitAttempted || credentialsCommitted) {
        try {
          if (previousCredentials) {
            await runPairingTransactionStep('connectXtreamProvider.rollback.setProviderCredentials(final)', () =>
              setProviderCredentials(providerId, previousCredentials!),
            { credentialKey: providerId },
            );
          } else {
            await runPairingTransactionStep('connectXtreamProvider.rollback.removeProviderCredentials(final)', () =>
              removeProviderCredentials(providerId),
            { credentialKey: providerId },
            );
          }
        } catch (rollbackError) {
          pairingTransactionFailure('connectXtreamProvider.rollback.credentials', rollbackError, { providerId });
        }
      }
