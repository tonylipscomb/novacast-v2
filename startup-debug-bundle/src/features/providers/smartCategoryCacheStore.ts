...
export const SMART_CATEGORY_CACHE_VERSION = 1;
...
export function getSmartCategoryCountSync(providerId: string, mediaType: MediaType, categoryKey: string) {
  return getSmartCategoryCacheSync(providerId, mediaType).entries[categoryKey]?.count ?? 0;
}

export function getSmartCategoryEntrySync(providerId: string, mediaType: MediaType, categoryKey: string) {
  return getSmartCategoryCacheSync(providerId, mediaType).entries[categoryKey];
}
