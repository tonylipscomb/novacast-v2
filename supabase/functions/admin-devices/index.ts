...
Deno.serve(async (request) => {
  ...
  try {
    ...
    let query = client.from('devices').select('id,public_device_code,friendly_name,platform,manufacturer,model,device_type,os_version,app_version,app_build,status,activation_status,last_seen_at,created_at,revoked_at').order('last_seen_at', { ascending: false, nullsFirst: false }).limit(Math.min(Number(url.searchParams.get('limit') ?? 100), 200));
