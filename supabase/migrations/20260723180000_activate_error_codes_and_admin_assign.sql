-- Distinct activation failure codes + admin assign-by-invite-id (no plaintext code needed).
...
-- Admin path: activate by invite UUID (no plaintext code). Same side effects as code redeem.
...
declare
  invitation public.beta_invites%rowtype;
begin
  select * into invitation from public.beta_invites where id = p_invite_id;
  if invitation.id is null then
    raise exception 'invite_not_found';
  end if;
